...
# 2. Data Cleaning
# -----------------------------
def handle_nulls(df):        #defines a function named handle_nulls that takes one argument df (expected to be a pandas DataFrame).
    print("Null Value Summary:\n", df.isnull().sum())  #prints a summary of how many missing (NaN) values
                                                        #are present in each column. df.isnull() produces a boolean DataFrame;
    df = df[df['customer_id'].notnull()].copy()  #filters df to only rows with a non-null

    # Fill numeric with mean                                        #.columns Index of numeric column names
    numeric_cols = df.select_dtypes(include=np.number).columns   #selects columns whose dtype is numeric (int, float, etc.).
    df[numeric_cols] = df[numeric_cols].apply(lambda x: x.fillna(x.mean()))

#lambda x, replace missing values with the column mean (x.mean()).
#Assigns the result back to the numeric columns in df. Effect: numeric NaNs get replaced by their column mean.
...
def handle_outliers(df):
    outlier_summary = {}      #track number of outliers found per numeric column.
    for col in df.select_dtypes(include=np.number).columns:
        Q1 = df[col].quantile(0.25)      #Q1 is the 25th percentile (first quartile) of column values.
        Q3 = df[col].quantile(0.75)     #q3 is the 75th percentile (third quartile).
        IQR = Q3 - Q1                   #iQR is interquartile range
        lower = Q1 - 1.5 * IQR
        upper = Q3 + 1.5 * IQR
        outliers = df[(df[col] < lower) | (df[col] > upper)]
        outlier_summary[col] = outliers.shape[0]      #ouliers.shape=counts how many outlier rows exist for this column.
                                                #outlier_summary=Stores that count in
...
def generate_insights(df):
    ...
    gender_premium = df.groupby('Gender')['annual_premium'].mean()    #how much the person pays annually for insurance).
    print("\nGender-wise Average Premium:\n", gender_premium)

    plt.figure(figsize=(6,4))  #sets the figure size in inches (width=6, height=4).
                                    # This ensures the plot is compact but readable.
